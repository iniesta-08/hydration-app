...
for i = 1:size(inputSets, 1)
    ...
    fitnessLevelCategory = getFitnessLevelCategory(outputData);
    fprintf('Input: Heart Rate = %d, Exercise Duration = %d, Step Count = %d\n', inputData(1), inputData(2), inputData(3));
    fprintf('Estimated Fitness Level: %.2f (%s)\n\n', outputData, fitnessLevelCategory);
end

% Function to determine fitness level category
function level = getFitnessLevelCategory(value)
    if value <= 25
        level = 'Poor';
    elseif value <= 50
        level = 'Average';
