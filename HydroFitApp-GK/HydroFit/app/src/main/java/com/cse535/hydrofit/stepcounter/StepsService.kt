...
class StepsService : LifecycleService(), SensorEventListener {
    ...
    private fun createNotification(
        ...
        val progress = if (goal == 0) 0.0 else (steps * 100 / goal)
        val calorieBurned = stats?.let {
            getCalorieBurned(
                it.height,
                it.weight, steps, it.pace
            )
        } ?: 0
        val distanceTravelled =
            stats?.let { getDistanceTravelled(steps, it.stepLength) } ?: 0
        val content =
            "${calorieBurned.toInt()} kcal · ${distanceTravelled.toInt()} km · $progress% of your daily goal"
    ...
    private fun getCalorieBurned(height: Int, weight: Int, steps: Int, pace: Int) = run {
